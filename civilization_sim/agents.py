...
class Person(Agent):
    def __init__(self, model, tribe_id=None):
        ...
        self.profession = self.random.choice(["Farmer", "Miner", "Guard", "Blacksmith", "Merchant", "Soldier", "Scholar", "Healer", "Priest"])
    ...
    def step(self):
        ...
        if self.infected:
            ...
            for neighbor in neighbors:
                if isinstance(neighbor, Person) and not neighbor.infected:
                    ...
                    if self.random.random() < 0.1:
                        ...
                        logging.info(f"Person {self.unique_id} infected Person {neighbor.unique_id} with plague")
            ...
            if self.random.random() < recovery_chance:
                self.infected = False
                logging.info(f"Person {self.unique_id} recovered from plague!")
        ...
        if self.energy <= 0:
            reason = "plague" if self.infected else "starvation"
            logging.info(f"Person {self.unique_id} from tribe {self.tribe_id} died of {reason} at {self.pos}")
        ...
        if self.age >= max_age:
            logging.info(f"Person {self.unique_id} from tribe {self.tribe_id} died of old age at {self.pos}")
            self.model.grid.remove_agent(self)
            self.model.schedule.remove(self)
            return
    ...
    def gather_wood(self):
        ...
        for agent in cell_mates:
            if isinstance(agent, Tree):
                ...
                logging.info(f"Person {self.unique_id} of tribe {self.tribe_id} gathered wood. Tribe wood: {self.model.tribe_stockpiles[self.tribe_id]['wood']}")
    ...
    def gather_stone(self):
        ...
        for agent in cell_mates:
            if isinstance(agent, Stone):
                ...
                logging.info(f"Person {self.unique_id} of tribe {self.tribe_id} gathered stone. Tribe stone: {self.model.tribe_stockpiles[self.tribe_id]['stone']}")
                self.model.grid.remove_agent(agent)
                self.model.schedule.remove(agent)
                break
    ...
    def gather_iron(self):
        ...
        for agent in cell_mates:
            if isinstance(agent, IronOre):
                ...
                logging.info(f"Person {self.unique_id} of tribe {self.tribe_id} gathered iron. Tribe iron: {self.model.tribe_stockpiles[self.tribe_id]['iron']}")
                self.model.grid.remove_agent(agent)
                self.model.schedule.remove(agent)
                break
    ...
    def work_smithy(self):
        ...
        if any(isinstance(agent, Smithy) for agent in cell_mates):
            ...
            if stockpile["iron"] >= 1:
                ...
                logging.info(f"Blacksmith {self.unique_id} produced tools. Tribe tools: {stockpile['tools']}")
    ...
    def work_library(self):
        ...
        if any(isinstance(agent, Library) for agent in cell_mates):
            ...
            logging.info(f"Scholar {self.unique_id} generated science. Tribe science: {stockpile['science']}")
                ...
                if isinstance(agent, Person) and agent.tribe_id == self.tribe_id:
                    if agent.infected:
                        if self.random.random() < 0.5:
                            ...
                            logging.info(f"Healer {self.unique_id} cured Person {agent.unique_id} of plague")
                    if agent.energy < 30:
                        agent.energy += 5
                        logging.info(f"Healer {self.unique_id} healed Person {agent.unique_id}")
        ...
        if any(isinstance(agent, Temple) for agent in cell_mates):
            ...
            if hasattr(self.model, "tribe_religion"):
                ...
                if rel == "Sun God": 
                    ...
                    logging.info(f"Priest {self.unique_id} generated science (Sun God). Tribe science: {stockpile['science']}")
            ...
            logging.info(f"Priest {self.unique_id} generated morale. Tribe morale: {stockpile['morale']}")
        ...
        if any(isinstance(agent, Tavern) for agent in cell_mates):
            ...
            if self.energy < 25:
                ...
                if stockpile["food"] >= 1:
                    ...
                    logging.info(f"Person {self.unique_id} ate at Tavern. Energy: {self.energy}")
    ...
    def attack_barbarian(self):
        ...
        if barbarians:
            ...
            logging.info(f"Defender {self.unique_id} attacked Barbarian at {target.pos}")
            if target.energy <= 0:
    ...
    def attack_predator(self):
        ...
        if predators:
            target = predators[0]
            self.energy -= 10
            logging.info(f"Guard {self.unique_id} killed Predator {target.unique_id} at {target.pos}")
            self.model.grid.remove_agent(target)
            self.model.schedule.remove(target)

    def attack_enemy(self):
        if self.profession != "Soldier" or self.tribe_id is None:
            return
        
        # Check for enemies in range (Soldiers have range 1 attack)
        neighbors = self.model.grid.get_neighbors(self.pos, moore=True, include_center=True, radius=1)
        enemies = [
        ...
        for enemy in enemies:
            ...
            if hasattr(self.model, "wars") and war_key in self.model.wars:
                ...
                logging.info(f"Soldier {self.unique_id} (Tribe {self.tribe_id}) attacked Enemy {enemy.unique_id} (Tribe {enemy.tribe_id}) at {enemy.pos}")
                
                if enemy.energy <= 0:
                    logging.info(f"Enemy {enemy.unique_id} killed by Soldier {self.unique_id}")
    ...
    def build_house(self):
        ...
        if stockpile["wood"] >= 15 and stockpile["stone"] >= 15 and stockpile.get("science", 0) == 0:
             if not any(isinstance(agent, (House, Farm, Wall, Smithy, Market, Road, Barracks, Library)) for agent in cell_mates):
                ...
                logging.info(f"Person {self.unique_id} from tribe {self.tribe_id} built a Library (Priority) at {self.pos}")
                return
        ...
        if stockpile["wood"] >= 15 and stockpile["stone"] >= 15:
             if not any(isinstance(agent, (House, Farm, Wall, Smithy, Market, Road, Barracks)) for agent in cell_mates):
                 ...
                 if tribe_pop > 5:
                    ...
                    logging.info(f"Person {self.unique_id} from tribe {self.tribe_id} built a Barracks at {self.pos}")
                    return
        ...
        if stockpile["wood"] >= 15 and stockpile["stone"] >= 15:
             ...
             if not any(isinstance(agent, (House, Farm, Wall, Smithy, Market, Barracks, Library)) for agent in cell_mates):
                 ...
                 if (stockpile["wood"] >= 15 and stockpile["stone"] >= 15 and stockpile.get("science", 0) == 0) or \
                    ...
                    self.model.schedule.add(library)
                    self.model.grid.place_agent(library, self.pos)
                    logging.info(f"Person {self.unique_id} from tribe {self.tribe_id} built a Library at {self.pos}")
                    return
        ...
        if stockpile["wood"] >= 10 and stockpile["stone"] >= 5:
             if not any(isinstance(agent, (House, Farm, Wall, Smithy, Market, Barracks, Library, Hospital, Temple, Tavern)) for agent in cell_mates):
                 ...
                 if stockpile["wood"] >= 12 and stockpile["stone"] >= 6:
                    ...
                    logging.info(f"Person {self.unique_id} from tribe {self.tribe_id} built a Hospital at {self.pos}")
                    return
    ...
    def trade(self):
        ...
        if market:
            ...
            # For now, let's simulate "Market Day" - trade with any tribe present
    ...
    def move(self):
        ...
        if target_type:
            ...
            if targets_near:
                ...
                best_step = min(possible_steps, key=lambda p: self.get_distance(p, target.pos))
                self.model.grid.move_agent(self, best_step)
                return
        ...
        # Prefer roads if moving randomly
        road_steps = []
        ...
        if road_steps and self.random.random() < 0.8: # 80% chance to follow road
             new_position = self.random.choice(road_steps)
        else:
...
class Predator(Agent):
    ...
    def reproduce(self):
        ...
        self.model.schedule.add(child)
        self.model.grid.place_agent(child, self.pos)
        logging.info(f"Predator {self.unique_id} from pack {self.pack_id} reproduced at {self.pos}")
        ...
        for agent in cell_mates:
            if isinstance(agent, (Person, House, Farm, Smithy, Market, Barracks, Library, Hospital, Temple, Tavern)):
                if isinstance(agent, Person):
                    agent.energy -= 20
                    self.energy += 10
                    logging.info(f"Barbarian attacked Person {agent.unique_id} at {self.pos}")
                    if agent.energy <= 0:
                        self.model.grid.remove_agent(agent)
                        self.model.schedule.remove(agent)
                else:
                    # Destroy building
                    logging.info(f"Barbarian destroyed {type(agent).__name__} at {self.pos}")
                    self.model.grid.remove_agent(agent)
                    self.model.schedule.remove(agent)
                return # Attack once per turn
